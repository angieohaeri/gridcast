...
- Hitting the mlflow UI from a laptop browser via the Tailscale hostname returned "Invalid Host header - possible DNS rebinding attack detected" even though the container was healthy - mlflow >=2.14 validates the `Host` header by default and rejects anything not explicitly allowed, unrelated to the app actually being up
- Fixed with `--allowed-hosts "localhost:*,127.0.0.1:*,mlflow:*,${KAFKA_TAILSCALE_HOST}:*"` on the `mlflow server` command in docker-compose.yml - needs the container-internal hostname (`mlflow`, used by `api`/`train.py`/`prefect-deployments` over `http://mlflow:5000`) in the same list as the Tailscale hostname, or fixing browser access alone would've broken every in-cluster client instead
...
- Right after the `--allowed-hosts` fix above, the `mlflow` container went "unhealthy" and stayed that way for 4-8+ minutes at a time, every single restart. Ruled out in order, each with actual evidence, not guesses: DB/network (`psycopg2.connect()` from the `api` container succeeded instantly, `pg_stat_activity` showed no blocking locks - two idle, already-committed connections), `--workers 4` (still hung at `--workers 1`), the new Host-header security middleware (still hung with `--disable-security-middleware`), resource starvation (`docker stats` showed mlflow's own container at 0.05% CPU / 200MB of a 3.7GB limit - a genuinely starved process would show some crawling CPU usage, not flat zero), uvloop (not even installed - bare `uvicorn` with no `[standard]` extras in `uv.lock`)
- The exact same command, same mlflow 3.14.0, against a real local Postgres backend, started clean in ~2 seconds outside this container - proved the mlflow/Postgres/workers/middleware combination isn't inherently broken, narrowing it to something about this specific Linux/Docker environment
- Root cause found via `py-spy dump --pid 1` (needed `cap_add: SYS_PTRACE`, temporary, since Docker blocks ptrace by default): the main thread was **actively executing**, not blocked - caught mid-`import pandas`, inside `_code_to_timestamp_pyc` (CPython's `.pyc` bytecode-cache step). Every `docker compose up -d mlflow` recreates the container from the image's layers with a fresh writable layer - any `.pyc` cache written during a *previous* container's lifetime doesn't persist, so every restart recompiled pandas + mlflow's many submodules from scratch. Normally a sub-second cost; here, competing for disk I/O with ~10 other containers on the same host (timescaledb alone showed 17.9GB of cumulative block I/O) stretched it into minutes and looked identical to a deadlock from the outside (near-zero CPU, no log progress, `futex_wait_queue` in `/proc/1/wchan` at the moments checked)
- Fixed at the source: `Dockerfile`'s `uv sync --frozen --no-dev` → `uv sync --frozen --no-dev --compile-bytecode`, which bakes `.pyc` files into the image layer at build time instead of leaving them to compile fresh on every container start. Applies to every service built from `gridcast:latest`, not just `mlflow` - the same slow-first-import cost was likely present (just less noticeable, since nothing else depends on those containers passing a healthcheck before startup is declared successful) on every producer/consumer/api/dashboard container too
- `--allowed-hosts`/`--disable-security-middleware` and the `cap_add: SYS_PTRACE` used to get the `py-spy` trace were diagnostic-only - reverted back to the real `--allowed-hosts` config and dropped the capability once root-caused
