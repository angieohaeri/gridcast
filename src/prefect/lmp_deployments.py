...
lmp_consumer.py. paused=True on every deployment per decisions.md, until each has a
validated run.
"""
...
if __name__ == "__main__":
    serve(
        ...
        sync_marginal_value_rt.to_deployment(
            name="raw_lmp_marginal_value_rt", schedule=Cron("15 12 * * *", timezone="America/New_York"), paused=True
        ),
        # Unconfirmed on PJM Data Miner - same settlement family as marginal_value_rt,
        # treated as daily.
        sync_marginal_value_da.to_deployment(
            name="raw_lmp_marginal_value_da", schedule=Cron("20 12 * * *", timezone="America/New_York"), paused=True
        ),
        # Unconfirmed - DA market clears once/day; treated as daily.
        sync_lmp_da_hourly.to_deployment(
            name="raw_lmp_lmp_da_hourly", schedule=Cron("25 12 * * *", timezone="America/New_York"), paused=True
        ),
        ...
        sync_generation_by_fuel.to_deployment(
            name="raw_lmp_generation_by_fuel", schedule=Cron("20 * * * *", timezone="America/New_York"), paused=True
        ),
        # Confirmed daily, 12-2pm ET.
        sync_transmission_constraints_da.to_deployment(
            name="raw_lmp_transmission_constraints_da", schedule=Cron("15 14 * * *", timezone="America/New_York"), paused=True
        ),
        # Confirmed monthly, posts on the 20th.
        sync_operator_initiated_commitments.to_deployment(
            name="raw_lmp_operator_initiated_commitments", schedule=Cron("0 5 21 * *", timezone="America/New_York"), paused=True
        ),
        # Unconfirmed - treated as daily, same as its rt_hrl_lmps-family siblings.
        sync_scheduled_generation.to_deployment(
            name="raw_lmp_scheduled_generation", schedule=Cron("30 12 * * *", timezone="America/New_York"), paused=True
        ),
        # Unconfirmed - treated as daily.
        sync_generation_ehv_losses.to_deployment(
            name="raw_lmp_generation_ehv_losses", schedule=Cron("35 12 * * *", timezone="America/New_York"), paused=True
        ),
        # Confirmed daily, 4:00am ET.
        sync_forecasted_generation_outages.to_deployment(
            name="raw_lmp_forecasted_generation_outages", schedule=Cron("15 4 * * *", timezone="America/New_York"), paused=True
        ),
        # EIA (not PJM), monthly data with a ~3-month reporting lag - weekly is plenty.
        sync_natural_gas_fuel_cost.to_deployment(
            name="raw_lmp_natural_gas_fuel_cost", schedule=Cron("0 3 * * 0", timezone="America/New_York"), paused=True
        ),
        ...
        raw_lmp_lmp_rt_unverified_fivemin_producer_flow.to_deployment(
            name="raw_lmp_lmp_rt_unverified_fivemin_producer", interval=timedelta(minutes=10), paused=True
        ),
        raw_lmp_lmp_rt_unverified_fivemin_consumer_flow.to_deployment(
            name="raw_lmp_lmp_rt_unverified_fivemin_consumer", interval=timedelta(minutes=10), paused=True
        ),
    )
