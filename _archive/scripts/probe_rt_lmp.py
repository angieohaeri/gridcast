...
# 1. What does REAL_TIME_HOURLY return right now, with the library's default row_is_current filter?
df = pjm.get_lmp(
    "2026-08-08",
    end="2026-08-10",
...
print("REAL_TIME_HOURLY (row_is_current filter, library default):")
print("  rows:", len(df), " max Interval Start:", df["Interval Start"].max() if len(df) else None)

# 2. Raw API call bypassing the library's row_is_current filter, to see if
# fresher rows exist but just aren't flagged current yet.
...
if len(raw):
    ...
    print(raw["row_is_current"].value_counts())
