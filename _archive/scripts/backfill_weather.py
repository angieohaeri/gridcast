...
# One-off backfill for the Mac Mini wifi outage / PJM rate-limit window: weather_producer
# has no historical range, so its 20-min poll misses aren't recoverable from live polling.
# Raw gap confirmed uniform across all 20 zones: 2026-09-17 16:45 UTC to 2026-09-20 00:45 UTC.
...
def main():
    ...
    conn.close()
    logger.success(f"Backfilled {written} weather rows for {START_DATE} to {END_DATE}")
