"""Backfill raw_lmp.scheduled_generation from pull_scheduled_generation.py's snapshot.

conf_disclaimer dropped - it's the static explanatory text for why rt_ecomax is null
(confidentiality suppression, ~55% of rows), not a data column.
"""

from loguru import logger
import pandas as pd
import psycopg2.extras

from gridcast.config import INTERIM_DATA_DIR, get_connection, setup_logging

setup_logging()

SNAPSHOT = INTERIM_DATA_DIR / "pjm_scheduled_generation_2023_present.csv"

UPSERT = """
INSERT INTO raw_lmp.scheduled_generation (time, rt_ecomax, self_ecomax)
VALUES %s
ON CONFLICT (time) DO UPDATE SET
    rt_ecomax = EXCLUDED.rt_ecomax,
    self_ecomax = EXCLUDED.self_ecomax
"""

raw = pd.read_csv(SNAPSHOT)
raw["time"] = pd.to_datetime(raw["Interval Start"], utc=True)
raw = raw.where(raw.notna(), None)
raw = raw.drop_duplicates(subset=["time"])

logger.info(f"{len(raw)} rows, {raw['time'].min()} -> {raw['time'].max()}")

rows = list(raw[["time", "rt_ecomax", "self_ecomax"]].itertuples(index=False, name=None))

conn = get_connection()
cur = conn.cursor()
psycopg2.extras.execute_values(cur, UPSERT, rows, page_size=5000)
conn.commit()
logger.success(f"upserted {len(rows)} rows")

cur.execute("select count(*), min(time), max(time) from raw_lmp.scheduled_generation")
logger.info(f"raw_lmp.scheduled_generation: {cur.fetchone()}")
conn.close()
